...
CLASS lcl_uml_class DEFINITION FRIENDS lif_unit_test.
  ...
  PRIVATE SECTION.
    ...
    METHODS link_class_name IMPORTING iv_name TYPE csequence
                            RETURNING VALUE(rv_name) TYPE string.
    METHODS hyperlink IMPORTING iv_uri TYPE string
                      RETURNING VALUE(rv_uri) TYPE string.
    METHODS link_module IMPORTING iv_name TYPE csequence
                        RETURNING VALUE(rv_name) TYPE string.
    METHODS get_uri IMPORTING iv_object TYPE trobjtype
                              iv_obj_name TYPE trobj_name
                    RETURNING VALUE(rv_uri) TYPE string.
    METHODS get_object_path EXPORTING ev_obj_name TYPE trobj_name
                                      ev_obj_type TYPE trobjtype
                                      ev_url_tail TYPE string.
    METHODS end_class.
...
INTERFACE lif_output.
  "! export data
  "! @parameter lcl_parameter | UML class scanner instance
...
CLASS lcl_uml_class IMPLEMENTATION.
  ...
  METHOD get_uri.
      TRY.
          rv_uri = cl_adt_uri_mapper=>get_instance( )->if_adt_uri_mapper~map_wb_object_to_objref(
             cl_wb_object=>create_from_transport_key( p_object = iv_object
                                                      p_obj_name = iv_obj_name ) )->get_http_url( ).
  ...
  METHOD get_object_path.
    CONSTANTS c_class_pool TYPE string VALUE `\CLASS-POOL=`.
    CONSTANTS c_prg_pool TYPE string VALUE `\PROGRAM=`.
    "CONSTANTS c_function_pool TYPE string VALUE `\FUNCTION-POOL=`.

    CLEAR ev_obj_name.
    ev_obj_type = 'CLAS'.
    ev_url_tail = '/source/main'.

    DATA(is_local) = xsdbool( ms_uml-container IS NOT INITIAL AND ms_uml-is_local EQ abap_true ).

    IF is_local = abap_true.
      IF find( val = ms_uml-container sub = c_class_pool ) <> -1.
        ev_obj_name = substring( val = ms_uml-container
                                 off = strlen( c_class_pool ) ).
        ev_url_tail = SWITCH #( substring( val = mv_name len = 4 )
                          WHEN 'LCL_' THEN '/includes/definitions'
                          WHEN 'LTCL' THEN '/includes/testclasses'
                          ELSE ev_url_tail ).
      "ELSE.
        " currently no links for local classes of FuGr and Programs possible
      ENDIF.
    ELSE.
  ...
  METHOD link_class_name.
    " Add Links https://plantuml.com/link
    get_object_path( IMPORTING ev_obj_name = DATA(lv_obj_name)
                               ev_obj_type = DATA(lv_object)
                               ev_url_tail = DATA(lv_url_tail) ).
    IF lv_obj_name IS NOT INITIAL.
      DATA(lv_uri) = get_uri( iv_object = lv_object
                              iv_obj_name = lv_obj_name ).
      IF lv_uri IS NOT INITIAL.
        lv_uri = | [[{ lv_uri && lv_url_tail }]] |.
      ENDIF.
    ENDIF.

    rv_name = iv_name && ` ` && lv_uri.
  ENDMETHOD.
  ...
  METHOD begin_class.
    ...
    mo_uml->add( |{ link_class_name( lv_name ) } \{\n| ).
    rv_flag = abap_true.
  ...
  METHOD hyperlink.
    rv_uri = COND #( WHEN iv_uri IS NOT INITIAL THEN | [[[{ iv_uri }]]]| ELSE '' ).
  ENDMETHOD.

  METHOD link_module.
    " Add Links cf https://plantuml.com/link
    IF find( val = ms_uml-name sub = '\FUGR=' ) GE 0.
      DATA(lv_url_trail) = escape( val = substring_before( val = iv_name sub = '( )' )
                                   format = cl_abap_format=>e_url_full ) && '/source/main'.
      DATA(lv_uri) = get_uri( iv_object = 'FUGR'
                              iv_obj_name = CONV #( mv_name ) ).
      IF lv_uri IS INITIAL.
        lv_uri = lv_url_trail.
      ELSE.
        lv_uri = lv_uri && '/fmodules/' && lv_url_trail.
      ENDIF.
    ENDIF.
    rv_name = iv_name && ` ` &&  hyperlink( lv_uri ).
  ENDMETHOD.
  ...
  METHOD uml_add.
    ...
    IF iv_class EQ abap_true.
      mo_uml->add( |\{static\}| ).
    ENDIF.
    mo_uml->add( link_module( iv_name ) && |\n| ).
  ENDMETHOD.                    "uml_add
...
*                     'I' - ?? Integer?
    rt_attr = VALUE #(
     ...
     ( ref = REF #( gs_cfg-svg )            text = 'SVG '(c32)                    kind = 'C' )
     ( ref = REF #( gs_cfg-display_source ) text = 'Display source '(c33)         kind = 'C' )    ).
  ENDMETHOD.
